...
export const StyleUploader: React.FC<StyleUploaderProps> = ({ onStyleAnalyzed }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      addLog("Initializing style extraction engine...");
      ...
      addLog("Analysis complete. Finalizing profile...");
      setTimeout(() => {
      ...
      console.error(err);
      setError("Failed to analyze style. Please try different images or check your connection.");
      ...
      {!isAnalyzing ? (
        <>
            <div className="mb-10 text-center">
                <h2 className="text-4xl font-bold text-white mb-3 tracking-tight font-display">Create Style Profile</h2>
                <p className="text-zinc-400 max-w-lg mx-auto leading-relaxed">
                    Upload 3-5 images that define your aesthetic. Gemini's vision model will analyze and extract the artistic DNA.
                </p>
            </div>
            ...
            <div className="flex-1 bg-zinc-900/30 border border-dashed border-zinc-700/50 rounded-3xl p-8 relative flex flex-col items-center justify-center min-h-[400px] transition-all hover:bg-zinc-900/50 hover:border-zinc-600 group/dropzone">
                ...
                <div className="text-center cursor-pointer p-10" onClick={() => fileInputRef.current?.click()}>
                    ...
                    <p className="text-zinc-200 font-semibold text-lg mb-2">Click to upload references</p>
                    <p className="text-zinc-500 text-sm">Supports JPG, PNG • Max 5 images</p>
                </div>
                ) : (
            ...
            <div className="mt-10 max-w-md mx-auto w-full">
                ...
                {images.length > 0 && images.length < 3 && (
                    <p className="text-center text-zinc-500 text-xs mt-4">
                        Upload <span className="text-zinc-300 font-bold">{3 - images.length}</span> more image{3 - images.length > 1 ? 's' : ''} to reach the minimum threshold
                    </p>
                )}
            </div>
        </>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-900/20 rounded-3xl border border-zinc-800/50 backdrop-blur-sm">
                ...
                <div className="relative bg-zinc-950 border border-zinc-800 rounded-2xl p-8 shadow-2xl overflow-hidden ring-1 ring-white/5">
                    ...
                    <div className="flex items-center justify-between mb-8 border-b border-zinc-800 pb-5">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-mono text-xs font-bold text-purple-300 tracking-wider">NEURAL ENGINE ACTIVE</span>
                        </div>
                        <span className="text-[10px] text-zinc-600 font-mono">v3.0.PRO</span>
                    </div>
                    ...
                    <div className="space-y-6 font-mono text-sm relative">
                        ...
                        <div className={`flex items-center gap-4 transition-all duration-500 ${logs.some(l => l.msg.includes("Preprocessing")) ? 'text-white translate-x-1' : 'text-zinc-600'}`}>
                            {logs.some(l => l.msg.includes("Preprocessing")) 
                            ...
                            <span className="font-semibold tracking-tight">Image Normalization</span>
                        </div>

                        {/* Step 2: Vector */}
                        <div className={`flex items-center gap-4 transition-all duration-500 ${logs.some(l => l.msg.includes("Extracting")) ? 'text-white translate-x-1' : 'text-zinc-600'}`}>
                            {logs.some(l => l.msg.includes("Extracting")) 
                                ? <div className="w-4 h-4 rounded-full bg-blue-500 flex items-center justify-center shadow-[0_0_10px_rgba(59,130,246,0.5)]"><Cpu size={10} className="text-white animate-spin-slow"/></div> 
                                : <div className="w-4 h-4 rounded-full border-2 border-zinc-700 bg-zinc-900" />}
                            <span className="font-semibold tracking-tight">Vector Extraction</span>
                        </div>

                        {/* Step 3: Semantic */}
                        <div className={`flex items-center gap-4 transition-all duration-500 ${logs.some(l => l.msg.includes("Synthesizing")) ? 'text-white translate-x-1' : 'text-zinc-600'}`}>
                            {logs.some(l => l.msg.includes("Synthesizing")) 
                                ? <div className="w-4 h-4 rounded-full bg-purple-500 flex items-center justify-center shadow-[0_0_10px_rgba(168,85,247,0.5)]"><ScanEye size={10} className="text-white"/></div> 
                                : <div className="w-4 h-4 rounded-full border-2 border-zinc-700 bg-zinc-900" />}
                            <span className="font-semibold tracking-tight">Semantic Analysis</span>
                        </div>
                    </div>
            ...
            <p className="mt-8 text-zinc-500 font-medium text-xs tracking-widest uppercase animate-pulse">
                Processing {images.length} Reference Images
            </p>
        </div>
      )}
    </div>
